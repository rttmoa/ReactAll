...
export function genLoanProduct(data) {
  ...
  const newData = [ ...sortArr.slice(0, 5), { name: '其它',value: remainSum } ];
...
export function genUserConver(data, legends) {
  ...
  const newData = Object.keys(data).map(item => {
    const sumItem = data[item].reduce((prev, cur) => (cur.value !== '-' ? cur.value + prev : prev), 0);
    if (item === 'regist') {
      sum = sumItem;
    }
    return {
...
export function genUserConverLine(data, legends) {
  return {
    ...
    tooltip: {
      formatter: params => {
        return `<p style="text-align:left;font-size:12px;line-height:18px">${params[0].name}</p>
          ${params.map(({ color, seriesName, value }) => {
              return value !== '-' ? `<p style="text-align:left;line-height:18px">
                  <i class="chart-circle" style="${styles}background-color:${color}"></i>
                  ${seriesName}：${value}
                </p>` : null;
            }).join('')}
