import {
  useRef,  useState,  useEffect,  createContext,  Fragment,  useCallback,  isValidElement,  useContext,
} from 'react'
...
// todo 此文件在   /layouts/ContentsLayout.js 被重写
export default function Document(props) {
  const router = useRouter()
  const { name = 'Missing title', tableOfContents, body, summary, collection__expand } = props;
          ...
          <div className="w-full flex-none lg:grid lg:grid-cols-3 lg:gap-8">
            <div className="bg-gray-50 lg:bg-transparent -mx-4 sm:-mx-6 lg:mx-0 py-12 sm:py-16 px-4 sm:px-6 lg:pl-0 lg:pr-8">
              <nav
                ...
                className="text-sm max-w-[37.5rem] mx-auto lg:max-w-none lg:mx-0 lg:fixed w-full h-full lg:pb-16 top-0 lg:pt-32 overflow-y-auto"
              ...
              >
                <ul role="list" className="space-y-8 border-l border-gray-200 pl-6">
                  {tableOfContents.map((section) => {
                    let sectionIsActive =
                      currentSection === section.slug || section.children.findIndex(({ slug }) => slug === currentSection) > -1;
                    return (
                      <Fragment key={section.slug}>
                        <li className="space-y-3">
                          <a href={`#${section.slug.toLowerCase()}`}
                            className="block font-medium transition-colors duration-300 text-teal-600"
                          >
                            {section.title}
                          </a>
                            ...
                            {section.children.map((subsection) => {
                              ...
                              return (
                                <li className="ml-4" key={subsection.slug}>
                                  <a href={`#${subsection.slug.toLowerCase()}`}
