...
export function Hero() {
  return (
    <Container className="pt-20 pb-16 text-center lg:pt-32">
      ...
      <p className="mt-6 max-w-3xl mx-auto text-lg text-slate-600 text-center dark:text-slate-400">
      ...
      <div className="mt-6 sm:mt-10   flex justify-center space-x-6 text-sm">
        <a href="/docs/" className="bg-slate-900 hover:bg-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50 text-white font-semibold h-12 px-6 rounded-lg w-full flex items-center justify-center sm:w-auto       dark:bg-sky-500 dark:highlight-white/20 dark:hover:bg-sky-400">
          快速开始
        </a>
            ...
            <>
              ...
              {actionKey && (
                <kbd className="font-sans font-semibold dark:text-slate-500">
                  <abbr title={actionKey[1]} className="no-underline text-slate-300 dark:text-slate-500">
                    {actionKey[0]}
                  </abbr>{` K`}
                </kbd>
              )}
      ...
      <div className="relative max-w-3xl mx-auto mt-10   px-4 sm:px-6 md:px-8 ">
        <div className='w-[360px] h-[200px] sm:w-[640px] sm:h-[360px] mx-auto aspect-video'>
          <Player
            // !外部盒子指定最大宽度，margin和padding
            // !内部盒子指定播放组件的宽高
            // ref={playerRef}
