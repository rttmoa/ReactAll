                ...
                <Tab.Panel key={category.name} className="focus:outline-none pt-10 pb-8 px-4 space-y-10">
                  ...
                  {category.sections.map((section) => ( 
                    ...
                    <div key={section.name}>
                      <p id={`${category.id}-${section.id}-heading-mobile`} className="">
                      ...
                      >
                        {section.items.map((item) => (
                          <li key={item.name} className="flow-root">
                            <a href={item.href} className="-m-2 p-2 block font-medium text-sm">
                              {item.name}
                            </a>
                          </li>
                        ))}
...
export function Header({ hasNav = false, navIsOpen, onNavToggle, title, section }) {
  ...
  return (
    <>
      <div className="hidden   py-2   bg-gradient-to-r from-sky-600 to-light-blue-500   overflow-hidden">
        ...
        <nav aria-label="Top" className="max-w-8xl mx-auto font-semibold text-base leading-6 ">
          <div className="py-4 border-b border-slate-900/10 lg:px-8 lg:border-0 dark:border-slate-300/10 mx-4 lg:mx-0">
            <div className="flex items-center">
              ...
              {/* TODO: Flyout menus: 产品/客户/文档/视频/联系我们 */}
                                    ...
                                    <div className="col-start-2 grid grid-cols-2 gap-x-8">
                                      {category.featured.map((item) => (
                                        <div key={item.name} className="group relative text-base sm:text-sm">
                                          ...
                                          <a href={item.href} className="mt-6 block font-semibold  text-slate-900 dark:text-slate-100">
                                            <span className="absolute z-10 inset-0" aria-hidden="true" />
                                            {item.name}
                                          </a>
                                          <p aria-hidden="true" className="mt-1">
