...
export function img({ node, ...props }) {
  ...
  return (
    <div className="relative pt-10 mt-10">
      <div className="absolute inset-x-0 bg-top bg-no-repeat beams top-0"></div>
...
export function a({ node, href, ...props }) {
  if (href && isString(href)) {
    ...
    if (result) {
      return <Frame className="aspect-video" src={href} {...props} />
    }
...
export function info({ node, ...props }) {
  return (
    <div className="block notice-block info">
      <div className="icon">
...
export function warning({ node, ...props }) {
  return (
    <div className="block notice-block warning">
      <div className="icon">
...
export function tip({ node, ...props }) {
  return (
    <div className="block notice-block tip">
      <div className="icon">
        <StarIcon></StarIcon>
      </div>
      <div className="content">{props.children}</div>
...
export function Markdown(props) {
  const { body, className = 'prose dark:prose-dark' } = props
