...
const UpdatedAt: React.FunctionComponent<{ date: string }> = ({ date }) => <div>{date}</div>
...
// todo 此文件内容为； 文档   /layouts/ContentsLayout.js   ==={文档布局}===
        ...
        <div className="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-2 lg:gap-x-8">
          {posts?.map((post) => {
            ...
            const imageUrl = post.image
              ? process.env.NEXT_PUBLIC_STEEDOS_ROOT_URL + `/api/files/images/${post.image}`
              : null
            return (
              <div key={post.slug} className="group relative flex flex-col overflow-hidden">
