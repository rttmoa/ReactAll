...
export async function getUserIdByToken(token, clientInfos = {}) { // ! 获取用户信息通过token
  let tokenMap = getTokenMapCache(token)
  if (!tokenMap) {
    let user = await getUser(token)
    if (user) {
      let userId = user._id
      setTokenMapCache(token, Object.assign({}, clientInfos, { userId: userId }))
      return userId
    } else {
      return
    }
...
export function removeUserTokens(userId, is_phone) { // ! 清除用户token
