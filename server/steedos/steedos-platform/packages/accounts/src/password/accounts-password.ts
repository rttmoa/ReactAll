...
import { User, LoginUserIdentity, EmailRecord, TokenRecord, DatabaseInterface, AuthenticationService, HashAlgorithm, ConnectionInformations, LoginResult } from '../types'
...
import { getUserResetTokens, getUserVerificationTokens, hashPassword, bcryptPassword, verifyPassword, isEmail } from './utils'
...
export default class AccountsPassword implements AuthenticationService {
  ...
  public async resetPassword(token: string, newPassword: PasswordType, infos: ConnectionInformations): Promise<LoginResult | null> {
    ...
    if (!resetTokenRecord || this.isTokenExpired(resetTokenRecord, resetTokenRecord.reason === 'enroll' ? this.options.passwordEnrollTokenExpiration : this.options.passwordResetTokenExpiration)) {
      throw new Error(this.options.errors.resetPasswordLinkExpired)
    }
    ...
    if (spaceUsers.length > 0) {
      ...
      if (profiles.length > 0) {
        ...
        if (_.has(userProfile, 'phone_login_expiration_in_days')) {
          phone_login_expiration_in_days = userProfile.phone_login_expiration_in_days
        }
      }
    }
  ...
  public async createUser(user: PasswordCreateUserType): Promise<string> {
    ...
    try {
      ...
      defer(async () => {
        if (this.options.sendVerificationEmailAfterSignup && user.email && !user.email_verified) this.sendVerificationEmail(user.email)
  ...
  public async passwordAuthenticator(user: string | LoginUserIdentity, password: PasswordType, isHashPassword = true): Promise<User> {
    const { username, email, id, mobile } = typeof user == 'string' ? this.toMobileAndEmail({ user }) : this.toMobileAndEmail({ ...user })
    ...
    if (!foundUser) {
      throw new Error(this.server.options.ambiguousErrorMessages ? this.options.errors.invalidCredentials : this.options.errors.userNotFound)
    }
    ...
    if (!isPasswordValid) {
      ...
      throw new Error(this.server.options.ambiguousErrorMessages ? this.options.errors.invalidCredentials : this.options.errors.incorrectPassword)
    } else {
  ...
  private async codeAuthenticator(user, token, locale): Promise<User> {
    const { username, email, mobile, id } = isString(user) ? this.toMobileAndEmail({ user }) : this.toMobileAndEmail({ ...user })

    let foundUser = await this.db.findUserByVerificationCode({ email: email, mobile: mobile }, token)
    if (!foundUser) {
      throw new Error(this.server.options.ambiguousErrorMessages ? this.options.errors.invalidCode : this.options.errors.userNotFound)
    }
    return foundUser
  }
