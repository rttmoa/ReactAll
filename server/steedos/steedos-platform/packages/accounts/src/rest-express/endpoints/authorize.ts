...
export const authorize = (accountsServer: AccountsServer) => async (req: express.Request, res: express.Response) => {
  ...
  let authToken = get(req.cookies, 'X-Auth-Token') || get(req.body, 'X-Auth-Token') || get(req.params, 'X-Auth-Token') || get(req.headers, 'Authorization') || get(req.headers, 'authorization')
  ...
  let userId = get(req.cookies, 'X-User-Id') || get(req.body, 'X-User-Id') || get(req.params, 'X-User-Id')
