...
import { User, LoginResult, Tokens, Session, ImpersonationResult, HookListener, DatabaseInterface, AuthenticationService, ConnectionInformations } from '../types'
...
export class AccountsServer {
  ...
  public async loginWithService(serviceName: string, params: any, infos: ConnectionInformations): Promise<LoginResult> {
    const hooksInfo: any = {
    ...
    try {
      ...
      if (userProfile) {
        ...
        phone_logout_other_clients = userProfile.phone_logout_other_clients || false
   ...
   */
  public async refreshTokens(accessToken: string, refreshToken: string, ip: string, userAgent: string): Promise<LoginResult> {
    try {
   ...
   */
  public createTokens({ token, isImpersonated = false, userId, name, email }: { token: string; isImpersonated?: boolean; userId: string; name: string; email: string }): Tokens {
  ...
  public prepareMail(to: string, token: string, user: User, pathFragment: string, emailTemplate: EmailTemplateType, from: string): any {
  ...
  private defaultPrepareEmail(to: string, token: string, user: User, pathFragment: string, emailTemplate: EmailTemplateType, from: string): object {
