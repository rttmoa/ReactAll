    ...
    static async visitors(ctx: any) {
        ...
        try {
            ...
            if (visitors && visitors.length > 0) {
                let newList = visitors.map((user: any) => { // 修改数据的： 相差距离，年龄差，缘分值
                    return {
                        ...user,
    ...
    static async questionSection(ctx: any) {
        try {
            ...
            let newdata = data.map((item: any) => {
                return {
                    ...item,
                    answers: ansdata.filter((t: any) => t.qsid == item.qsid).sort((p: any, n: any) => { return p.ans_No.charCodeAt() - n.ans_No.charCodeAt() })
                }
            })
