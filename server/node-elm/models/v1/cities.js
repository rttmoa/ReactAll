...
citySchema.statics.cityGuess = function (name) { // ? 城市猜测
...
citySchema.statics.cityHot = function () { // ? 最热城市
  return new Promise(async (resolve, reject) => {
    try {
      const city = await this.findOne()
...
citySchema.statics.cityGroup = function () { // ? 城市组
  return new Promise(async (resolve, reject) => {
    try {
      const city = await this.findOne()
...
citySchema.statics.getCityById = function (id) { // ? 通过iD获取城市
  return new Promise(async (resolve, reject) => {
    try {
      const city = await this.findOne()
      Object.entries(city.data).forEach(item => {
