...
import { ORDER_DEPART } from '../utils/constant.js';
...
/***--- 坐席类型, 车次类型, 出发车站, 到达车站中每一项Item    修改下{name}123即可 ---**/
...
const BottomModal = memo(function BottomModal(props) {
    ...
    /***--- 坐席类型：特，一，二 ---**/
    const [localCheckedTicketTypes,localCheckedTicketTypesDispatch] = useReducer(checkedReducer, checkedTicketTypes, checkedTicketTypes => checkedTicketTypes);

    /***--- 车次类型：D，T，K，Z ---**/
    const [localCheckedTrainTypes, localCheckedTrainTypesDispatch] = useReducer(checkedReducer,checkedTrainTypes,checkedTrainTypes => checkedTrainTypes);

    /***--- 出发车站 ---**/
    const [localCheckedDepartStations,localCheckedDepartStationsDispatch] = useReducer(checkedReducer,checkedDepartStations,checkedDepartStations => checkedDepartStations);

    /***--- 到达车站 ---**/
    const [localCheckedArriveStations,localCheckedArriveStationsDispatch] = useReducer(checkedReducer,checkedArriveStations,checkedArriveStations => checkedArriveStations);
    ...
    const isResetDisabled = useMemo(() => {
        return (
            Object.keys(localCheckedTicketTypes).length === 0 && Object.keys(localCheckedTrainTypes).length === 0 &&
    ...
    // {title: '坐席类型', options: Array(10), checkedMap: {…}, dispatch: ƒ} 
    // {title: '车次类型', options: Array(5), checkedMap: {…}, dispatch: ƒ} 
    // {title: '出发车站', options: Array(2), checkedMap: {…}, dispatch: ƒ} 
    // {title: '到达车站', options: Array(2), checkedMap: {…}, dispatch: ƒ} 
...
export default function Bottom(props) {
    ...
    const {
        ...
        ticketTypes, trainTypes, // 座位类型，火车类型
        departStations, arriveStations, // 出发车站，到达车站
        checkedTicketTypes, checkedTrainTypes, checkedDepartStations, checkedArriveStations,
        departTimeStart,departTimeEnd,arriveTimeStart,arriveTimeEnd,
    ...
    const noChecked = useMemo(() => {
        return (
            Object.keys(checkedTicketTypes).length === 0 && Object.keys(checkedTrainTypes).length === 0 &&
            Object.keys(checkedDepartStations).length === 0 && Object.keys(checkedArriveStations).length === 0 &&
            departTimeStart === 0 && departTimeEnd === 24 && arriveTimeStart === 0 && arriveTimeEnd === 24
        );
