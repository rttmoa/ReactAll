...
function Articles() {
  ...
  // console.log(form)/**--- 方法：获取表单字段值、获取表单值、重置表单、提交、校验、设置表单值、getInnerMethods、getTouchedField ---**/
  const dispatch = useDispatch();
  const [tagsArr, setTagsArr] = useState([]);/**--- 标签 ---**/
  const [categoriesArr, setCategoriesArr] = useState([]);/**--- 文章分类 ---**/

  const getTags = async () => {/**--- 获取标签数据 使用map加工数组 返回新数据 ---**/
    const res: any = await getTagsList({
      page: 1,
      pageSize: 9999,
    });
    const list = res.data.list?.map((item) => {/**--- 加工数组 返回新数组 ---**/
      item.key = item._id;
  ...
  const getCategories = async () => {/**--- 获取文章分类数据 使用map加工数组 返回新数据 ---**/
  ...
  const columns = [
    ...
    {
      title: '文章标题',
      dataIndex: 'title',//属性名=data数据中的属性名
    },
  ...
  const articlesState = useSelector((state: ReducerState) => state.articles);/**--- REDUX获取数据 ---**/
  ...
  // Form表单：获取表单值、更改表单值属性、调用函数发请求重新获取新数据
  const onSearch = async () => {/**--- 获取表单值、更改表单值属性、调用函数发请求重新获取新数据 ---**/
    ...
    if (postData.tags) {
      postData.tags = postData.tags.join(',');/**--- ['ES7', 'CSS'] --> "ES6,ES7" ---**/
    }
    ...
    fetchData(1, pagination.pageSize, postData);/**--- 参数1：当前值1，参数2：页码数， 参数3：请求参数 ---**/
  };
  ...
  const handleUpdateCollectStatus = async (isCollect) => {/**--- 单选框传递true/false 发请求 重新获取数据 ---**/
  ...
  const layout = {/**--- 样式：标签、包装  labelCol、wrapperCol ---**/
    labelCol: {
      span: 8,
    },
    wrapperCol: {
  ...
  return (
    <div className={styles.container}>
      ...
      <Card bordered={false}>
        ...
        {/* Form: https://arco.design/react/components/Form  表单的使用 */}
        <Form
          form={form}
          initialValues={{
            categories: '',
