    ...
    try {
      ...
      if (response.code == 200 && response.data.length > 0 && response.data[response.data.length - 1].hashFrom == hash) {
        ...
        api.info({
          ...
          message: 'Done',
          description: (
            <a target="_blank" href={`${getBlockExplorerUrls(currentToChain.id)}/tx/${response.data[0].hashTo}`} className={styles.tokenlist}>
    ...
    <div style={{ padding: '50px 0' }} className="flex flex-center">
      ...
      <div
        ...
        <div className="flex flex-between flex-align-center" style={{ marginBottom: '15px' }}>
          ...
          {/* 先隐藏 */}
          <div style={{ cursor: 'pointer' }} onClick={() => setVisibleSetting(true)}>
            <SettingOutlined />
          </div>
        ...
        <Drawer
          ...
          <div className={styles.tokenlist}>
            {(isAll &&
              getTokenList(null).map((item: any) => (
                <div className={styles.item} onClick={() => onSelectTokenCurrent(item)}>
                  <div>
              ...
              getTokenList(null)
                .filter((item: any) => item.name != 'USDT')
                .map((item: any) => (
                  <div className={styles.item} onClick={() => onSelectTokenCurrent(item)}>
                    <div>
                      <img src={item.icon} style={{ marginRight: '15px' }} />
        ...
        {/* 选择链 */}
        <Drawer title="Select  Chain" className={styles.h100} placement="bottom" onClose={() => setOpen(false)} open={open} getContainer={false} maskClosable={false} style={{ position: 'absolute' }} bodyStyle={{ paddingTop: '0' }}>
          <div className={styles.tokenlist}>
            {chainList.map(item => (
              <div className={styles.item} onClick={() => onSelectChainCurrent(item)}>
                <div>
                  <img src={item.icon} style={{ marginRight: '15px' }} />
                  <p>{item.name}</p>
                </div>
          ...
          <div className={styles.tokenlist}>
            <div className="flex flex-between flex-align-center">
              ...
              <Tooltip
                ...
                title="The default amount allows you to perform a couple of transactions (e.g. Approve and Swap). Once you approve the transfer in your wallet, the transaction gas amount will be higher than a regular transaction as this includes the selected amount of destination gas to be sent.">
                <InfoCircleOutlined />
              </Tooltip>
            </div>
            ...
            <Input
              ...
              prefix={
                <span>
                  <img src="/eth.svg" style={{ marginRight: '5px', width: '20px' }} />
                </span>
              }
              suffix={<span>Max</span>}
            />
            <div className="flex flex-between flex-align-center" style={{ marginTop: '30px' }}>
              <span> Slippage tolerance</span>
              <Tooltip overlayStyle={{ width: 300 }} title="Your transaction will revert if the price changes unfavorably by more than this percentage">
                <InfoCircleOutlined />
              </Tooltip>
            </div>
            <hr style={{ background: '#323232', margin: '10px 0px 20px 0px' }} />
            ...
            <Input style={{ marginTop: '20px' }} placeholder="0.0" suffix={<span>%</span>} />
          </div>
        </Drawer>
      </div>
    </div>
