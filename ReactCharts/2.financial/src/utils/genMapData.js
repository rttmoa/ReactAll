...
export function genOverviewMap(cmap, message) {
  ...
  const min = (sortData[sortData.length - 1] && sortData[sortData.length - 1].value.toFixed(0)) || 0;
  effectScatterSeries[i] = { ...effectScatterSeries[i],
    data: [
      {
        ...message,
        visualMap: false,
...
export function genOverviewBar(cmap) {
  ...
  const top10 = sortData.filter((f, index) => index < 10).map(item => {
      return {
        ...item,
